...
$script:DirectToolNamePatterns = @(
  ...
  "^(tox|nox|quarto|crystal|xcodebuild|bazel|buck2)(\.exe)?$",
  "^(jupyter|jupyter-lab|jupyter-notebook)(\.exe)?$",
  "^(gradle(w)?(\.bat)?|mvn(\.cmd)?)$",
  "^(sbt(\.bat)?|scala(-cli)?(\.cmd|\.exe)?|kotlin(\.exe)?)$",
  "^(clj|lein)(\.bat|\.cmd|\.exe)?$",
  "^(ghci|runghc|cabal|stack)(\.exe)?$",
  "^(dune|prove|cpanm|luarocks|zig)(\.exe)?$",
  "^(air|reflex)(\.exe)?$",
...
$script:WorkspaceScopedShellPatterns = @(
  ...
  "\b(tox|nox)(\.exe)?\b.*(\b(-e|--workdir|--noxfile|-s)\b|[/\\].*\.tox\b|[/\\].*noxfile\.py\b)",
  "\bquarto(\.exe)?\b.*\b(render|preview|publish|check)\b",
  "\bcrystal(\.exe)?\b.*\b(spec|run|build)\b",
  "\bxcodebuild(\.exe)?\b.*(\.xcodeproj|\.xcworkspace|\b-project\b|\b-workspace\b).*\b(test|build|archive)\b",
  "\b(bazel|buck2)(\.exe)?\b.*\b(test|build|run)\b",
  ...
  "\bclj(\.bat|\.cmd|\.exe)?\b.*((?<![A-Za-z])-M(?=[:\s])|(?<![A-Za-z])-X(?=[:\s])|(?<![A-Za-z])-T(?=[:\s])|(?<![A-Za-z])-m\b|\b(test|run|repl)\b)",
  "\blein(\.bat|\.cmd|\.exe)?\b.*\b(test|run|repl|uberjar)\b",
  "\b(cabal|stack)(\.exe)?\b.*\b(test|run|build|repl|ghci)\b",
  "\b(ghci|runghc)(\.exe)?\b.*\.l?hs\b",
  "\bdune(\.exe)?\b.*\b(runtest|test|exec|build)\b",
  "\bprove(\.exe)?\b.*\.t\b",
  "\bcpanm(\.exe)?\b.*(--test-only|--local-lib-contained|--local-lib|install|test)\b",
  "\bluarocks(\.exe)?\b.*\b(test|build|install)\b",
  "\bzig(\.exe)?\b.*\b(build|test|run)\b",
  "\bdotnet(\.exe)?\b.*\b(watch|run|test|build|publish)\b",
  "\bjulia(\.exe)?\b.*(\.jl\b|--project(?:=|\s+)[^\s]+)",
  "\b(go(\.exe)?)\b.*\b(run|test)\b",
  "\b(air|reflex)(\.exe)?\b",
  "\b(rscript|perl|lua|ocaml)(\.exe)?\b.*[/\\].*\b(dev|test)\b.*\.(r|pl|lua|ml)\b",
